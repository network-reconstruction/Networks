...
class DirectedS1EnsembleAnalyser:
    ...
    def __init__(self,
        ...
        self.gen = gen
        self.parent_dir = "outputs" #parent directory for the output files
    ...
    def run_analysis(self, 
        ...
        Args:
            ...
            parent_dir (str): Parent directory for the output files. Defaults to "outputs".
        ...
        for _ in range(num_samples):
            ...
            reciprocity, clustering, in_degree_sequence, out_degree_sequence, generation_time = self.gen.calculate_metrics()
            ...
            self.generation_time_list.append(generation_time)
        
        self.parent_dir = parent_dir    
        if not os.path.exists(parent_dir):
            os.makedirs(parent_dir)  
        if not os.path.exists(f"{parent_dir}/{self.gen.output_rootname}"):
            os.makedirs(f"{parent_dir}/{self.gen.output_rootname}")
        if not os.path.exists(f"{parent_dir}/{self.gen.output_rootname}/ensemble_analysis"):
            os.makedirs(f"{parent_dir}/{self.gen.output_rootname}/ensemble_analysis")
        if save_results:
            self._save_data()
    ...
    def _save_data(self) -> None:
        ...
        with open(f"{self.parent_dir}/{self.gen.output_rootname}/ensemble_analysis/results.json", 'w') as f:
    ...
    def _plot_degree_distribution(self) -> None:
        ...
        if not os.path.exists(f"{self.parent_dir}/{self.gen.output_rootname}/ensemble_analysis/figs"):
            os.makedirs(f"{self.parent_dir}/{self.gen.output_rootname}/ensemble_analysis/figs")
        ...
        plt.yscale('log')
        plt.tight_layout()
        plt.savefig(f"{self.parent_dir}/{self.gen.output_rootname}/ensemble_analysis/figs/complementary_cumulative_degree_distribution.png")
        ...
        plt.grid(True)
        plt.savefig(f"{self.parent_dir}/{self.gen.output_rootname}/ensemble_analysis/figs/in_vs_out_degree_distribution.png")
