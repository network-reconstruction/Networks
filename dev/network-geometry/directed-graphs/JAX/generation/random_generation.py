...
class DirectedS1Generator:
    ...
    def __init__(self,
        ...
        self.NU = -10
        self.R = -10
        self.parent_dir = "outputs" # parent directory for all outputs
    ...
    def calculate_metrics(self) -> Tuple[float, float, List[int], List[int]]:
        ...
        Returns:
            Tuple[float, float, List[int], List[int], float]: The reciprocity, clustering coefficient, in-degree sequence, out-degree sequence, and generation time.
        """
        if self.verbose:
        ...
        return self.reciprocity, self.clustering, self.in_degree_sequence, self.out_degree_sequence, self.generation_time
    ...
    def get_output(self) -> dict:
        ...
        Output contains the following:
            ...
            generation_time (float): The time taken to generate the network.
    ...
    def _save_data(self) -> None:
        """
        Saves output data to <self.parent_dir>/<output_rootname>/generation_data.json.
        ...
        """
        if self.verbose:
            self.logger.info(f"Saving data to {self.parent_dir}/{self.output_rootname}/generation_data.json")
        output = self.get_output()
        
        if not os.path.exists("outputs"):
            os.makedirs("outputs")  
        if not os.path.exists(f"{self.parent_dir}/{self.output_rootname}"):
            os.makedirs(f"{self.parent_dir}/{self.output_rootname}")
        with open(f"{self.parent_dir}/{self.output_rootname}/generation_data.json", 'w') as f:
            json.dump(output, f, indent=4)
            
    def _save_coordinates(self) -> None:
        """
        Save the coordinates of the nodes to <self.parent_dir>/<output_rootname>/coordinates.csv.
        """
        if self.verbose:
            self.logger.info(f"Current working directory: {os.getcwd()}")
            self.logger.info(f"Saving coordinates to {self.parent_dir}/{self.output_rootname}/coordinates.csv")
        #node in_kappa, out_kappa, theta save as dataframe
        if not os.path.exists(self.parent_dir):
            os.makedirs(self.parent_dir)  
        if not os.path.exists(f"{self.parent_dir}/{self.output_rootname}"):
            os.makedirs(f"{self.parent_dir}/{self.output_rootname}")
        with open(f"{self.parent_dir}/{self.output_rootname}/coordinates.csv", 'w') as f:
